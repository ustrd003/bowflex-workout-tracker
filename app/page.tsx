...
import { accessoryExercises, exercises, getInitialProgramSelection, strengthDays, weeks, workoutDays } from "./workout-data";
...
export default function Home() {
  ...
  const dayExercises = [...exercises, ...(accessoryExercises[day] ?? [])];
  ...
  useEffect(() => { const target = !isStrength ? plan.cardio : plan.strengthE95; setCardioMinutes(target.match(/\d+/)?.[0] ?? "20"); setCardioResistance(target.match(/level (\d+)/)?.[1] ?? "2"); }, [week, day, isStrength, plan.cardio, plan.strengthE95]);
  ...
  const saveWorkout = async () => {
    ...
    await saveCardio(false); setStatus("Workout saved"); await refresh();
  };
  ...
  const completedDays = useMemo(() => new Set([...progress.workouts.map((x) => `${x.week}-${x.day}`), ...progress.cardio.map((x) => `${x.week}-${x.day}`)]), [progress]);
  const completedCount = completedDays.size;
  const exercise = dayExercises[activeExercise];

  return <main>
    <header className="topbar"><div className="brand-mark"><Dumbbell /></div><div><p className="eyebrow">8-week plan</p><h1>Bowflex Workout Tracker</h1></div><div className="header-progress"><span>{completedCount} of 40 workouts</span><Progress value={(completedCount / 40) * 100} /></div></header>
    <div className="app-shell">
      <section className="week-strip" aria-label="Choose workout">
        ...
        <label>Week<Select value={String(week)} onValueChange={(value) => setWeek(Number(value))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{weeks.map((item) => <SelectItem key={item.week} value={String(item.week)}>Week {item.week}</SelectItem>)}</SelectContent></Select></label>
        <label>Workout<Select value={day} onValueChange={setDay}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{workoutDays.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></label>
        <div className="plan-pill"><Flame /><span><strong>{isStrength ? `${plan.sets} set${plan.sets > 1 ? "s" : ""} · ${plan.reps} reps` : "Cardio day"}</strong>{isStrength ? `E95 finish: ${plan.strengthE95}` : plan.cardio}</span></div>
      </section>
      <Tabs defaultValue="workout" className="main-tabs">
        ...
        <TabsContent value="workout">
          {isStrength ? <div className="workout-grid">
            ...
            <CardioPanel planText={plan.strengthE95} minutes={cardioMinutes} resistance={cardioResistance} setMinutes={setCardioMinutes} setResistance={setCardioResistance} onSave={() => saveCardio()} compact />
          </div> : <CardioPanel planText={plan.cardio} minutes={cardioMinutes} resistance={cardioResistance} setMinutes={setCardioMinutes} setResistance={setCardioResistance} onSave={() => saveCardio()} />}
        ...
        <TabsContent value="progress"><div className="progress-layout">
          ...
          <section className="history-card"><div className="section-heading"><div><p className="eyebrow">Consistency</p><h2>Eight-week progress</h2></div><strong>{Math.round((completedCount / 40) * 100)}%</strong></div><div className="history-table"><div className="history-row history-head"><span>Week</span>{workoutDays.map((item) => <span key={item}>{item.slice(0, 3)}</span>)}<span>Weight</span></div>{weeks.map((item) => { const weightEntry = progress.weights.find((x) => x.week === item.week); return <div className="history-row" key={item.week}><strong>{item.week}</strong>{workoutDays.map((workoutDay) => <span key={workoutDay} className={completedDays.has(`${item.week}-${workoutDay}`) ? "complete-dot" : "empty-dot"}>{completedDays.has(`${item.week}-${workoutDay}`) ? <Check /> : "—"}</span>)}<span>{weightEntry ? `${weightEntry.weight} lb` : "—"}</span></div>; })}</div></section>
        </div></TabsContent>
        <TabsContent value="guide"><section className="plan-card"><p className="eyebrow">Your weekly rhythm</p><h2>Build up gradually</h2><p className="guide-intro">Strength on Monday, Wednesday and Friday. E95 cardio on Tuesday and Saturday. Monday: Goblet Squat and Calf Raise. Wednesday: Romanian Deadlift and Glute Bridge. Friday: Lateral Raise and Step-Up. Rest—or take an easy 10–20 minute walk—on Thursday and Sunday.</p><div className="plan-table"><div className="plan-row plan-head"><span>Week</span><span>Bowflex</span><span>After strength</span><span>Cardio days</span></div>{weeks.map((item) => <div className={item.week === week ? "plan-row current" : "plan-row"} key={item.week}><strong>{item.week}</strong><span>{item.sets} set{item.sets > 1 ? "s" : ""} × {item.reps}</span><span>{item.strengthE95}</span><span>{item.cardio}</span></div>)}</div><div className="safety-note"><HeartPulse /><p><strong>Safety:</strong> Use the lowest step height initially and keep stable support within reach for calf raises and step-ups. Stop for joint pain, chest pain, faintness, or unusual shortness of breath. Increase dumbbell weight only after completing every repetition with controlled form and feeling capable of another two or three repetitions.</p></div></section></TabsContent>
      </Tabs>
    </div>
  </main>;
}

function CardioPanel({ planText, minutes, resistance, setMinutes, setResistance, onSave, compact = false }: { planText: string; minutes: string; resistance: string; setMinutes: (value: string) => void; setResistance: (value: string) => void; onSave: () => void; compact?: boolean }) {
  return <section className={compact ? "cardio-card compact" : "cardio-card"}><div className="cardio-orb"><HeartPulse /></div><div className="cardio-copy"><p className="eyebrow">SOLE E95</p><h2>{compact ? "Cardio finish" : "Today’s cardio"}</h2><p>Target: <strong>{planText}</strong>. Keep a smooth pace and use the talk test.</p></div><div className="cardio-fields"><label>Minutes<Input type="number" min="1" value={minutes} onChange={(e) => setMinutes(e.target.value)} /></label><label>Resistance<Input type="number" min="1" max="20" value={resistance} onChange={(e) => setResistance(e.target.value)} /></label><Button onClick={onSave}><Check />Save cardio</Button></div></section>;
}
